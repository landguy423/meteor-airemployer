...
Collections.createThumbnails = function(collection, fileRef, cb) {
  var finish, isLast;
  ...
  finish = function(error) {
    return bound(function() {
      if (error) {
        console.error("[Collections.createThumbnails] [finish]", error);
      } else {
        if (isLast) {
          cb && cb(fileRef);
        }
    ...
    return bound(function() {
      ...
      return image.size(function(error, features) {
        return bound(function() {
          var i;
          if (error) {
            throw new Meteor.Error("[Collections.createThumbnails] [_.each sizes]", error);
          }
          i = 0;
          ...
          return _.each(sizes, function(size, name) {
            ...
            copyPaste = function() {
              fs.copy(fileRef.path, path, function(error) {
                return bound(function() {
                  var upd;
                  if (error) {
                    console.error("[Collections.createThumbnails] [_.each sizes] [fs.copy]", error);
                  } else {
                    upd = {
                      $set: {}
                    };
            ...
            if (!!~['jpg', 'jpeg', 'png'].indexOf(fileRef.extension.toLowerCase())) {
              ...
              updateAndSave = function(error) {
                return bound(function() {
                  if (error) {
                    console.error("[Collections.createThumbnails] [_.each sizes] [img.resize]", error);
                  } else {
                      ...
                      return bound(function() {
                        gm(path).size(function(error, imgInfo) {
                          return bound(function() {
                            var upd;
                            if (error) {
                              console.error("[Collections.createThumbnails] [_.each sizes] [img.resize] [fs.stat] [gm(path).size]", error);
                            } else {
                              upd = {
                                $set: {}
                              };
